!!! $Id: matutils.F90,v 1.48 2016/10/20 14:28:07 mjlaine Exp $
!!! ------------------------------------------------------------------------
...
!#define EPPES 1  ! uncomment this if compiling the EPPES version
  ...
  character(len=*), parameter :: utils_version="$Revision: 1.48 $ Compiled: "//__DATE__
  ...
  character(len=*), parameter :: gfrmstr = 'G20.11'
...
!!! 
  subroutine writedata_mat(file,xmat,stat,uselock)
    ...
    if (uselock2) then
       call open_with_lock(file=file,unit=101,status='replace',iostat=fstat)
    else
#ifdef _WIN32
       open(unit=101, file=file, status='replace', &
            recordtype='stream_cr', iostat=fstat)
#else
       open(unit=101, file=file, status='replace',  iostat=fstat)
#endif
    end if
    if (fstat /= 0) then
    ...
    do i=1,m
       do j=1,n
!         write(101,'('//gfrmstr//',x)', iostat=fstat, advance='NO') real(xmat(i,j))
          write(101,'('//gfrmstr//',x)', iostat=fstat, advance='NO') xmat(i,j)
          if (fstat /= 0) then 
    ...
    call close_with_lock(unit=101,file=file,iostat=fstat,uselock=uselock2)
...
!   call writedata(file,xmat,stat)
    ...
    do i=1,n
       write(101,'('//gfrmstr//')', iostat=fstat, advance='YES') xvec(i)
       if (fstat /= 0) then 
          write(*,*) 'Write error on file ', trim(file)
          stop 
       end if
...
!    character(len=*), parameter :: spaces = achar(32)//achar(44)//achar(59)//achar(9) ! spaces
    ...
    call open_with_lock(file=file,unit=101,status='old',iostat=fstat,uselock=uselock2)
    if (fstat /= 0) then
    ...
    ok = .false.
    do while (.not. ok)
       read(101,"(A)",iostat=fstat) line ! maybe use size=count here?
       if (fstat /= 0) then
          write(*,*) 'Error reading the first line of ', trim(file)
          if (present(status)) then
             status = fstat
             call close_with_lock(unit=101,file=file,uselock=uselock2)
             return
          else
             stop
          end if
