...
program mcmcmain
 ...
 use driver, only : acdc_driver ! driver
  ...
  call mcmc_main()
  !call RENAME('chain.dat', 'chain_10p_1.dat')
  !call RENAME('sschain.dat', 'sschain_10p_1.dat')
   !call RENAME('mcmcmean.dat', 'mcmcmean_10p_1.dat')
...
function ssfunction(theta,npar,ny) result(ss)
  ...
  real(kind=dbl) :: ss(ny), f(357),alpha(10),ss1(1)
  ...
  interface
     function modelfunction(theta,x,f,data) result(y)
       use mcmcprec
       implicit none
       real(kind=dbl) :: theta(:), x(:), y(size(x)),f(357)
       real(kind=dbl), pointer :: data(:,:)
 ...
 ! if (checkbounds(theta)) then
  ...
  modoutp(1:size(modoutp)) = modelfunction(theta,data(:,1),f,data)
  ss(1) = 0
  do i = 1,size(data(:,2))
...
function modelfunction(theta,x,f,data) result(y)
  ...
  real(kind(1.d0)) :: conc_final(nclust), conv, coef(40) ,ss(1), sigma
  real :: start, finish
  real(kind=dbl), pointer :: data(:,:)
